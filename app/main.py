...
# Force load secrets from local Dell server .env
ENV_FILE = ".env"
...
class SentinelV10_2:
    ...
    def load_settings(self):
        if os.path.exists(SETTINGS_FILE):
            with open(SETTINGS_FILE, "r") as f:
                ...
                self.balance_paper = saved.get("balance_paper", 10237.00) #
        else: self.mode, self.spread_active, self.balance_paper = "PAPER", True, 10237.00
    ...
    async def get_market_data(self, ticker):
        async with httpx.AsyncClient() as client:
            try:
                ...
                return {"bid": d.get('yes', [[0,0]])[-1][0], "ask": 100 - d.get('no', [[0,0]])[-1][0], "liq": d.get('no', [[0,0]])[-1][1]}
            except: return None

    async def risk_manager_loop(self):
        while True:
            # Inventory Skew: Sell faster as we fill slots
            ...
            for i, pos in enumerate(self.positions):
                ...
                if m and m['bid'] >= (pos['entry_price'] + (skew * 100)):
                    ...
                    self.positions.pop(i); self.save_settings()
                    await self.log_event(f"SKEW-EXIT: {pos['market_id']} | Net: ${profit:.2f}", "sell")
                    break
    ...
    async def execute_trade(self, ticker, price):
        ...
        self.positions.append({"market_id": ticker, "entry_price": price, "size": 100.00, "tag": self.mode, "entry_time": datetime.now()})
        self.save_settings()
...
async def get_status():
    return {"balance_paper": engine.balance_paper, "stats": {"spread": {"active": engine.spread_active}}, "positions": engine.positions, "vault_loaded": bool(engine.api_key)}

@app.post("/save_secrets")
async def save_secrets(api_key: str = Form(...), priv_key: str = Form(...)):
    engine.save_secrets(api_key, priv_key)
    await engine.log_event("VAULT: Secrets saved to Dell Server.", "system")
    return {"status": "success"}

@app.post("/toggle")
async def toggle(strat: str = Form(...), active: str = Form(...)):
    engine.spread_active = (active.lower() == 'true')
    engine.save_settings(); return {"status": "success"}

@app.get("/")
async def index(request: Request): return templates.TemplateResponse("index.html", {"request": request, "v": "10.2"})
